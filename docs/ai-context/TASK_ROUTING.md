...
| Report rendering | `src/renderers/decisionSummary.ts`, `src/renderers/markdownNotes.ts`, `src/renderers/markdownReport.ts`, `src/renderers/markdownSummary.ts` |
| Repository scanning / classification | `src/repo/fileClassifier.ts`, `src/repo/getChangedFiles.ts`, `src/repo/ignoredChangedFiles.ts`, `src/repo/index.ts` |
...
| --- | --- | --- | --- | --- |
| CLI flags/output | `src/cli/index.ts`, `src/cli/initGuardian.ts`, `src/cli/runGuardian.ts` | core command handler, README examples, CLI tests | none detected | Keep output stable for tests and scripts. |
...
| Report rendering | `src/renderers/decisionSummary.ts`, `src/renderers/markdownNotes.ts`, `src/renderers/markdownReport.ts`, `src/renderers/markdownSummary.ts` | renderers, snapshot-like tests, README examples | `tests/markdownReport.test.ts` | Keep generated sections deterministic. |
| Repository scanning/classification | `src/repo/fileClassifier.ts`, `src/repo/getChangedFiles.ts`, `src/repo/ignoredChangedFiles.ts`, `src/repo/index.ts` | context file rules, symbol map output, scan tests | `tests/fileClassifier.test.ts`, `tests/getChangedFiles.test.ts`, `tests/ignoredChangedFiles.test.ts` | Avoid full source reads except bounded symbol extraction. |
...
| GitHub Actions / release workflow | `.github/workflows/guardian-self-check.yml`, `.github/workflows/test.yml`, `RELEASE_NOTES.md`, `src/analyzers/releaseAnalyzer.ts` | package scripts, workflow files, release docs | `tests/releaseAnalyzer.test.ts` | Use Investigation Mode before changing deploy or release behavior. |
